...
module.exports = function(vantage, options) {
  ...
  vantage
    .command("__authenticate", "Authentication strategy.")
    .hidden()
    .action(function(args, cb) {
      var self = this;
      this._authenticate(args, function(err, authenticated) {
  ...
  vantage
    ...
    .action(function(args, cb) {
      ...
      this.client.connect(server, port, options, function(err, data) {
        cb(err, data);
      });
    });
