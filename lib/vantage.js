...
vantage._onKeypress = function(key, value) {
  if (this.session.isLocal() && !this.session.client) {
    var result = ui.getKeypressResult(key, value);
    if (result !== undefined) { 
...
vantage.prompt = function(options, cb) {
  ...
  if (ssn.isLocal()) {
    ui.setDelimiter(options.message || self._delimiter);
    ui.prompt(options, function(result) {
      ui.setDelimiter(this._origdelimiter);
      cb(result);
    });
  } else {
...
vantage._exec = function(item) {
  ...
  ui.history((item.session._mode ? modeCommand : item.command));
