...
/**
 * Registers a "catch" command in the vantage API.
...
vantage.catch = function(name, desc, opts) {
...
vantage._onKeypress = function(key, value) {
  ...
  if (this.session.isLocal() && !this.session.client) {
    this.session.getKeypressResult(key, value, function(err, result) {
      if (result !== undefined) {
        if (_.isArray(result)) {
          var formatted = VantageUtil.prettifyArray(result);
          self.ui.imprint();
...
vantage._exec = function(item) {
  ...
  if (match) {
    ...
    var parsedBasic = VantageUtil.parseArgs(args);
    ...
    var valid = true;
    for (var i = 0; i < supportedArgs; ++i) {
      if (origArgs[i]) {
        valid = (!valid) ? false : validateArg(parsedArgs._[i], origArgs[i]);
        if (!valid) { break; }
        parseArgsByType(parsedArgs._[i], origArgs[i]);
      }
    }
 ...
 * Vantage exits with 0 by default
 * on a sigint.
 *
 * @param {Function} fn
 * @return {Vantage}
...
vantage.sigint = function(fn) {
  if (_.isFunction(fn)) {
    ui.sigint(fn);
  } else {
    throw new Error('vantage.sigint must be passed in a valid function.');
  }
  return this;
};

/**
 * Removes a given command from the list of
 * commands.
 *
 * @param {String} cmd
 * @return {Vantage}
...
vantage.removeCommand = function(name) {
  this.commands = _.reject(this.commands, function(command){
...
/**
 * Hides a given command from the list of commands.
 *
 * @param {String} cmd
 * @return {Vantage}
 * @api public
...
vantage.hideCommand = function(name) {
  this.commands = _.map(this.commands, function(command){
    if (command._name === name) {
