...
vantage.prompt = function(options, cb) {
  ...
  if (self.is('local') && !self.is('server')) {
...
vantage._execQueueItem = function(cmd) {
  ...
  if (self.is('local')) {
    ...
    self._command = cmd;
    self._send('vantage-command-upstream', 'upstream', { command: cmd.command, args: cmd.args, completed: false });
  };
};
...
vantage._exec = function(item) {
  ...
  if (match) {
    ...
    var res = fn.call(this, args, function() {
...
vantage._authenticate = function(args, cb) {
  ...
  this._authFn.call(self, args, function(msg, authenticated) {
    if (self.session) {
      self.session.authenticating = false;
      self.session.authenticated = authenticated;
    }
