...
function Vantage() {
  ...
  //this._origdelimiter = this._delimiter;
  //this._delimiterCache = this._delimiter;
...
vantage.connect = function(server, port, options, cb) {
  console.log("PARENT CLIENT CONNECT")
...
vantage._prompt = function(data) {
  ...
  ui.prompt({
    ...
    if (self._cancelled == true) { self._cancelled = false; return; } // to do 
...
vantage._execQueueItem = function(cmd) {
  ...
  if (cmd.session.isLocal() && !cmd.session.client) {  // to do we have to make sure we kill the client when it dies.
    ...
    console.log("SENDING ANOTHER COMMAND!!")
    self._command = cmd;
...
vantage._exec = function(item) {
  ...
  console.log("CALLING EXEC COMMAND..." + item.command)
  ...
  if (match) {
    ...
    if (res && _.isFunction(res.then)) {
      return res.then(function(data){
        ...
        item.session.log(["", '  Error: '.red + err, '']);
        if (self.is('local')) {
