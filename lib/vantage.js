...
vantage._tempDelimiter = function(str, options) {
  ...
  if (!ssn || ssn.isLocal()) {
    ...
    if (this._midPrompt) {
      this._refresh({ sessionId: (ssn || {}).id });
    }
  } else {
...
vantage.show = function() {
  ...
  this.hook();
  this._prompt({ sessionId: this.session.id });
  return this;
};
...
vantage.prompt = function(options, cb) {
  ...
  if (ssn.isLocal()) {
    self._tempDelimiter((options.message) ? options.message : self._delimiter, { sessionId: options.sessionId });
    self._midPrompt = true;
    inquirer.prompt(options, function(result) {
      self._midPrompt = false;
      self._tempDelimiter(self._origdelimiter, { sessionId: options.sessionId });
      cb(result);
    });
  } else {
...
vantage._refresh = function(options) {
...
vantage._prompt = function(data) {
  ...
  var ui = inquirer.prompt({
    ...
    if (str == "") { self._prompt(data); return; }
...
vantage._execQueueItem = function(cmd) {
  ...
  if (cmd.session.isLocal() && !cmd.session.client) {  // to do we have to make sure we kill the client when it dies.
    //console.log('OK HERES TEH SESSION', cmd.session.client)
    this._exec(cmd);
  } else {
    self._command = cmd;
...
vantage._exec = function(item) {
  ...
  this._hist.push((item.session._mode ? modeCommand : item.command));
...
vantage._authenticate = function(args, cb) {
  ...
  if (!this._authFn) { 
    ...
    cb("No authentication required.", true);
  } else {
    this._authFn.call(self, args, function(msg, authenticated) {
      ssn.authenticating = false;
      ssn.authenticated = authenticated;
      cb(msg, authenticated);
    });
  }
};
...
vantage._send = function(str, direction, data, options) {
  ...
  if (direction == "upstream") {
    if (!ssn.client) {
      console.log("WTF NO SESSION CLIENT".red, ssn.id, ssn.isLocal());
      throw new Error("No session client on vantage send upstream.");
    }
...
vantage._keypressHandler = function(e, prompt) {
  ...
  if (this.session.isLocal()) {
    if (keyMatch) {
      ...
      if (this.session._mode) {
        this._exitMode({ sessionId: this.session.id });
        return;
      }
    } else {
      this._histCtr = 0;
...
vantage.exit = function(options, cb) {
  var self = this;
  console.log('HI EXIT', options)
  var ssn = this.getSessionById(options.sessionId);
  if (ssn.isLocal()) {
    console.log('BOOM', options)
    if (options.force) {
      process.exit(1);
    } else {
      this.prompt({
    ...
    for (var i = 0; i < self.server.sessions.length; ++i) {
      ...
      ssn.server.emit("vantage-close-downstream");
    }
  }
};
