...
vantageServer.listen = function(cb) {
  ...
  this.io.on('connection', function(socket) {
    ...
    console.log("SERVER SHALLOW CONNECT")
    ...
    on('vantage-keypress-upstream', function(data) {
      self.parent._proxy('vantage-keypress-upstream', 'upstream', data).then(function(){
        if ((['up', 'down', 'tab'].indexOf(data.key) > -1)) {
          ...
          self.parent._send('vantage-keypress-downstream', 'downstream', { value: response, sessionId: ssn.id });
        } else {
    ...
    if (self.parent._banner) {
      ssn.server.emit('vantage-banner-downstream', {
        banner: self.parent._banner,
      });
    }

    ssn.server.emit('vantage-heartbeat-downstream', {
      delimiter: self.parent._delimiter,
    });
