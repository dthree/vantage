...
vantageServer._auth = function(handshakeData, cb) {
...
vantageServer.listen = function(cb) {
  ...
  this.io.on('connection', function(socket) {
    ...
    session.io.authenticating = true;
    session.io.authenticated = false;
            ...
            return new Promise(function(resolve, reject){
              var cmd = {
                ...
                callback: function() {
                  ...
                  self.parent._send('vantage-command-downstream', 'downstream', { command: data.command, completed: true, error: void 0, data: arguments });
                },
              }
    ...
    on('vantage-close-upstream', function(data) {
      self.parent._proxy('vantage-close-upstream', 'upstream', data).then(function() {
        self.parent._send('vantage-close-downstream', 'downstream');
      });
    }); 
    ...
    self.hook(function(txt) {
      ...
      if (txt && txt.length > 1 && stripAnsi(txt).length == 0) { return ""; }
    ...
    function on(str, opts, cbk) {
      ...
      cbk = cbk || function() {}
      opts = opts || {}
      session.io.on(str, function() {
        if (!session.io || (!session.io.authenticating && !session.io.authenticated)) { 
